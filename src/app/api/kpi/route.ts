...
import { allowedAgg, ranges, Agg, Range } from "@/data/navigationMap";
...
export async function GET(req: NextRequest) {
  ...
  if (!allowedAgg.includes(agg as Agg)) {
  ...
  const pick = fake[metric as keyof FakeData];
  if (!pick) return NextResponse.json({ error: "Unknown metric" }, { status: 400 });

  const value = pick[agg as keyof MetricData];
